...
describe('Searching for a recipe', () => {
   ...
   it("Searching for a recipe (API test)", () => {
    ...
    it("Searching for a recipe that does not exist", () => {
        ...
        cy.wait(500)
        cy.get("[data-test='FailedSearch']", { timeout: 60000 }).should("contain", "No recipes that match your search term and/or dietary filters were found. Please try a different keyword or filter.")
        cy.get("[data-test='OkFailedSearch']", { timeout: 60000 }).click()
    })

    it("Searching for a recipe based off my dietary preferences, search not on homepage routes to homepage ", () => {
        cy.viewport(1280, 720)
        cy.get("[data-test='Dropdown']", { timeout: 60000 }).click()
        cy.get("[data-test='DietaryRestrictions']", { timeout: 60000 }).click()
        cy.get("[data-test='DietSelect']", { timeout: 60000 }).click()
        cy.get("[data-test='Pescetarian']", { timeout: 60000 }).click()
        cy.get("[data-test='AddTag']", { timeout: 60000 }).click()

        cy.get("[data-test='DietFilters']", { timeout: 60000 }).click()
        cy.get("[data-test='checkbox']", { timeout: 60000 }).first().click()
        cy.get("[data-test='CloseDietFilters']", { timeout: 60000 }).click()
        cy.get("[data-test='SearchBar']", { timeout: 60000 }).type("til")
        cy.get("[data-test='SearchButton']", { timeout: 60000 }).click()
        cy.wait(1000)
